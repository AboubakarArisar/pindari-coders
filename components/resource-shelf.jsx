...
export default function ResourceShelf() {
  ...
  return <section aria-label="Resource collection" aria-busy={feed.state === 'loading'}>
    ...
    {feed.state === 'loading' && !feed.items.length && <div className="resource-grid resource-skeleton-grid" aria-hidden="true">{Array.from({ length: 12 }, (_, index) => <article className="resource-skeleton" key={index}><div className="skeleton-top"><i /><i /></div><div className="skeleton-logo"><span /></div><div className="skeleton-copy"><b /><i /><i /></div></article>)}</div>}
    {feed.items.length > 0 && <>
      ...
      <div ref={resultsRef} className="resource-results" style={{ scrollMarginTop: '110px' }}>
        {results.total > 0 ? <div className="resource-grid">{results.items.map((resource, index) => <article className="resource-card" key={resource.id} style={{ '--card-index': index }}><a href={resource.url} target="_blank" rel="noopener noreferrer" aria-label={`Visit ${resource.name}`}><div className="resource-card-top"><div className="news-topic-tags">{resource.category && <span>{resource.category}</span>}{resource.pricing && <span>{resource.pricing}</span>}</div><span className="resource-arrow" aria-hidden="true">↗</span></div><div className="resource-logo"><img src={resource.image} alt={resource.alt} width="512" height="512" loading="lazy"/></div><div className="resource-card-body"><h2>{resource.name}</h2><p>{resource.description}</p><span className="resource-visit">open resource <b aria-hidden="true">↗</b></span></div></a></article>)}</div> : <div className="resource-no-results"><p>No resources match these filters yet.</p><button className="text-button" onClick={() => { setFilters({ category: 'All', pricing: 'All' }); setPage(1); }}>show all resources ↗</button></div>}
