...
export default function TrendingFeed() {
  ...
  return <section className="news-feed" aria-label="AI, tech and job market feed" aria-busy={feed.state === 'loading'}>
    ...
    <div role="status" className={feed.state === 'loading' && !feed.stories.length ? 'sr-only' : 'feed-status'}>{feed.state === 'loading' && 'Fetching recent stories from Hacker News and DEV Community…'}{feed.state === 'error' && <p>We couldn’t refresh the feed. {page.total ? 'Previous results still within the last 72 hours are shown below.' : 'Please try refreshing again.'}</p>}{feed.state === 'ready' && feed.failed.length > 0 && <p>Some requests to {feed.failed.join(' and ')} failed. Available results are shown; coverage may be incomplete.</p>}</div>
    {feed.state === 'loading' && !feed.stories.length && <div className="news-skeletons" aria-hidden="true">{Array.from({ length: 5 }, (_, index) => <div className="news-skeleton" key={index}><span /><div><i /><b /><i /></div></div>)}</div>}
    {(feed.state !== 'loading' || feed.stories.length > 0) && <h2 className="news-results-heading" tabIndex={-1} ref={resultsHeading}>{page.total} matching {page.total === 1 ? 'story' : 'stories'}</h2>}
